...
public class GamePanel extends JPanel implements ActionListener {
    ...
    Timer specialAppleTimer; // Timer for the special apple lifespan
    final int specialAppleLifespan = 5000; // 5 seconds in milliseconds
    ...
    public void startGame() {
        ...
        timer.start();
        specialAppleTimer = new Timer(specialAppleLifespan, new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                if (specialApple != null) {
                    specialApple = null; // Remove the special apple after the lifespan
                    spawnSpecialApple(); // Spawn a new special apple
                }
            }
        });
    ...
    public void draw(Graphics g) {
        if (running) {
            ...
            // Draw special apple if present
            if (specialApple != null) {
                g.setColor(Color.yellow);
                g.fillOval(specialApple.getX(), specialApple.getY(), object_size, object_size);
            }
            // Snake
    ...
    public void checkApple() {
        ...
        // Check if the snake eats the special apple
        if (specialApple != null && (x[0] == specialApple.getX()) && (y[0] == specialApple.getY())) {
            bodyParts++;
            applesEaten += 2; // Grant 2 points
            specialApple = null; // Remove the special apple
        }
    }

    public void checkCollisions() {
        // Check if head touches the borders (of the JFrame)
        ...
        if (!running) {
            timer.stop();
            specialAppleTimer.stop(); // Stop the special apple timer
        }
    }
    ...
    public void spawnSpecialApple() {
        if (specialApple == null && random.nextInt(100) < 5) { // 5% chance to spawn a special apple
            int specialAppleX = random.nextInt((int)(width / object_size)) * object_size;
            int specialAppleY = random.nextInt((int)(height / object_size)) * object_size;
            specialApple = new SpecialApple(specialAppleX, specialAppleY);
        }
    }
